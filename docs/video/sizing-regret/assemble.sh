#!/usr/bin/env bash
#
# Kordovan — assemble "sizing-regret"
# 9:16 Meta ad, 15.0s, 5 shots, all generated, captions burned in.
#
# Usage:   ./assemble.sh
# Input:   ./raw/shot01.mp4 … ./raw/shot05.mp4   (LTX output, 704x1280, ~4.04s each)
#          ./captions.srt
#          ./audio/bed.m4a                        (optional — silence if absent)
#          ./fonts/                               (optional — Fraunces .ttf files)
# Output:  ./out/sizing-regret.mp4
#
...
W=1080          # Meta 9:16
H=1920
FPS=30          # LTX renders at 24; normalise before concat
...
#
# Every source clip is 97 frames @ 24fps = 4.04s, so each has headroom over
# its target duration below and the trim never runs off the end of a file.
# ---------------------------------------------------------------------------
SHOTS=(shot01 shot02 shot03 shot04 shot05)
DURS=(  3.0     2.8     2.8     2.8     3.6 )
#       Sept    Jan     gap     resolve close   -> 15.0s
...
CAP_LINE="&H00161C24"     # Espresso   — outline, so text survives a misty frame
...
#
# The concat demuxer does a stream copy: it will produce a corrupt or desynced
# file if the parts differ in resolution, pixel format, frame rate or SAR.
# Every clip here comes out of LTX at 704x1280 @ 24fps, so all five are
# upscaled and retimed to 1080x1920 @ 30fps first, at CRF 18 — one stop better
# than the delivery CRF so this intermediate is not the quality floor.
# ---------------------------------------------------------------------------
i=0
for s in "${SHOTS[@]}"; do
  dur="${DURS[$i]}"
  ...
  # without letterboxing. 704x1280 and 1080x1920 are both 0.5625, so the crop
  # is a no-op today — it is there so a re-render at another size still lands.
  # lanczos is the sharpest practical scaler and matters on a 1.53x upscale.
  #
  # setsar=1 forces square pixels; a non-1 SAR makes concat refuse the file.
  # -an drops audio: generated clips are silent anyway and the bed is added
  # once, at the end.
...
#   # then set the Style field of cues 5 and 6 to Price
